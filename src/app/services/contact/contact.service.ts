...
export class ContactService {
  ...
  getAllContacts(): Observable<ContactList[]> {
    ...
    return this.http.post<ContactList[]>(
      `${environment.baseURL}user/contactslist/`,
      user
    );
  }
  ...
  inviteContact(contactMail: string): Observable<ContactList[]> {
    const donnees = {
      ...
      inviteur: localStorage['user'],
    };
    return this.http.post<ContactList[]>(
      `${environment.baseURL}user/invitecontact`,
      donnees
    );
  }
  ...
  updateListeDemandeEnvoyee(contactMail: string): Observable<ContactList[]> {
    ...
    return this.http.post<ContactList[]>(
      `${environment.baseURL}user/updatedemandeenvoyee`,
      donnees
    );
  }
  ...
  getInvitEnAttente() {
    ...
    return this.http.post<ContactList[]>(
      `${environment.baseURL}user/getinvitattente`,
      donnees
    );
  }
  ...
  accepterAmi(contactMail: string) {
    const donnees = {
      contactMail: contactMail,
      accepteur: localStorage['user'],
    };
    return this.http.post<ContactList[]>(
      `${environment.baseURL}user/accepterami`,
      donnees
    );
  }
  ...
  getAllFriends(): Observable<ContactList[]> {
    const donnees = { inviteur: localStorage['user'] };
    return this.http.post<ContactList[]>(
      `${environment.baseURL}user/getallfriends`,
      donnees
    );
  }
  ...
  deleteFriend(contactMail: string) {
    ...
    return this.http
      .post(`${environment.baseURL}user/deletefriend/`, donnees)
      .subscribe();
  }
  ...
  getinscrits() {
    return this.http.get(`${environment.baseURL}user/getinscrits/`);
  }
